  ...
  providers: [
    CredentialsProvider({
      ...
      async authorize(credentials) {
        ...
        // Check all roles
        const admin = await prisma.admin.findFirst({ where: { username: credentials.username as string } });
        ...
        const teacher = await prisma.teacher.findFirst({ where: { username: credentials.username as string } });
        ...
        const student = await prisma.student.findFirst({ where: { username: credentials.username as string } });
        ...
        const parent = await prisma.parent.findFirst({ where: { username: credentials.username as string } });

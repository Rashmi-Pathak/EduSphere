...
import { auth } from "@/lib/clerk-mock-server";
...
export async function getSubjectPageSettings(
  ...
  try {
    ...
    return {
      ...
      sectionsOrder: (settings.sectionsOrder ? JSON.parse(settings.sectionsOrder as string) : [
        "assignments",
        "exams",
        "materials",
      ]),
  ...
  try {
    ...
    await prisma.subjectPageSettings.upsert({
      ...
      create: {
        ...
        sectionsOrder: JSON.stringify(parsed.data.sectionsOrder),
      },
      update: {
        announcement: parsed.data.announcement ?? null,
        description: parsed.data.description ?? null,

            ...
            <div className="top-full inset-x-0 z-10 absolute bg-white shadow-xl mt-2 border border-gray-200 rounded-lg max-h-56 overflow-y-auto">
              {filteredItems.length > 0 ? (
                filteredItems.map((item) => (
                  <label
                    key={item.id}
                    className="flex items-center gap-2 hover:bg-eduspherePurpleLight px-4 py-3 w-full hover:text-eduspherePurpleDark text-sm text-start transition-colors cursor-pointer"
                  >
                    <input
                      type="checkbox"

...
export default function AiEvaluationCard({
  ...
  return (
    <div className="rounded-md border border-eduspherePurpleLight bg-white p-3">
      <div className="flex flex-wrap items-center justify-between gap-2">
        <div className="flex items-center gap-2 text-sm font-semibold text-eduspherePurpleDark">
        ...
        <button
          ...
          className="inline-flex items-center gap-2 rounded-md bg-eduspherePurpleDark px-3 py-1.5 text-xs font-medium text-white transition hover:brightness-90 disabled:cursor-not-allowed disabled:opacity-60"
        >
      ...
      {evaluation?.status === "FAILED" && (
        <div className="mt-3 flex gap-2 rounded-md bg-red-50 p-3 text-xs text-red-700">
          ...
          <span>{evaluation.error ?? t("manualFallback")}</span>
        </div>
      )}
      ...
      {evaluation && evaluation.status !== "FAILED" && (
        <div className="mt-3 space-y-3">
          <div className="flex flex-wrap items-center gap-2">
            <span className="rounded-md bg-eduspherePurpleLight px-2 py-1 text-xs font-semibold text-eduspherePurpleDark">
              {t("appliedDraftScore", {
          ...
          <p className="text-sm text-gray-700 whitespace-pre-wrap">
            {evaluation.feedback ?? t("noFeedback")}
          </p>

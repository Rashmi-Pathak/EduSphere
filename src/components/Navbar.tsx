...
import { useUser } from "@/lib/clerk-mock-client";
      ...
      <div className="flex min-w-0 flex-1 items-center gap-2 sm:gap-3">
        {onMenuClick && (
          <button
            ...
            onClick={onMenuClick}
            className="lg:hidden flex flex-shrink-0 justify-center items-center bg-eduspherePurpleDark hover:bg-eduspherePurpleDark/80 shadow-sm border border-eduspherePurple/25 rounded-md w-10 h-10 text-white transition"
          >
        ...
        {onMenuCollapseToggle && (
          <button
            ...
            className="hidden lg:flex flex-shrink-0 justify-center items-center bg-eduspherePurpleDark hover:bg-eduspherePurpleDark/70 shadow-sm border border-eduspherePurple/25 rounded-md w-10 h-10 text-white transition"
          >
            {isRtl ? (
              isMenuCollapsed ? (
        ...
        {schoolName && authUser?.role !== "superAdmin" && (
          <div className="ms-0 flex min-w-0 items-center gap-3 rounded-md bg-gradient-to-r from-edusphereYellow via-edusphereYellow/50 to-edusphereYellow/30 px-3 py-2 text-xs sm:ms-2 sm:px-4 sm:py-3">
            <span className="min-w-0 truncate text-[13px] font-bold uppercase text-eduspherePurpleDeep sm:text-[16px]">
              {schoolName}
            </span>
          </div>
        )}
        ...
        <div className="flex items-center gap-2 sm:gap-3">
          ...
          {canViewMessages && (
            ...
            >
              ...
              {messageCount > 0 && (
                <div className="-top-2.5 absolute flex justify-center items-center bg-eduspherePurpleDark rounded-full w-5 h-5 text-white text-xs -end-1.5">

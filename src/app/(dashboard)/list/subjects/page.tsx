      ...
      {data.length === 0 ? (
        ...
        <div className="gap-4 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 mt-6">
          {data.map((item: SubjectList) => (
            <article
              key={item.id}
              className="group relative flex flex-col justify-between bg-gradient-to-br from-eduspherePurpleDark via-eduspherePurple to-eduspherePurpleLight hover:shadow-md p-5 rounded-xl min-h-[150px] transition-shadow"
            >
              <Link
